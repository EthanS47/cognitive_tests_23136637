...
def single_ans_test():
    ...
    form_url2 = 'https://docs.google.com/forms/d/e/1FAIpQLScjAQkdm71BeZnITszXRvPCET0SVcTIKvlJ_zlg2l-ZvjdvnA/viewform?usp=sf_link'
    random.seed(1)
    ...
    user_answers_dict2['user_id'] = user_answers_dict['user_id']
    clear_output(wait=True)
    ...
    You will only have 3 seconds to choose an answer.\n
    ...
    # Running test again but showing ANS test images for longer period of time
    random.seed(2)
    random.shuffle(images_list)
    clear_output(wait=False)
    score2 = 0
    total_time2 = 0
    print("The second test will now start in:")
    time.sleep(2.5)
    print("3")
    time.sleep(1)
    print("2")
    ...
    for i in images_list:
        ...
        display(blank_placeholder)
        # Recording time taken for each question
        user_answers_dict2[time_id] = time_taken
        ...
        if result['description'] == 'Left':
            ...
            if answer == ans_answers_global[i]:
                score2 += 1
                user_answers_dict2[question_id] = True
            elif answer != ans_answers_global[i]:
                user_answers_dict2[question_id] = False
            ...
            print("No button was pressed! Answer more quickly")
            user_answers_dict2[question_id] = 'NA'
    ...
    user_answers_dict2['total_time'] = total_time2
    send_to_google_form(user_answers_dict, form_url)
    send_to_google_form(user_answers_dict2, form_url2)
    clear_output(wait=False)
    print(f"Your approximation for the first test was {score} of {len(images_list)}.")
    print(f"Your approximation for the second test was {score2} of {len(images_list)}.")
